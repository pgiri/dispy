...
class DispyNetRelay(object):
    ...
    def relay_pings(self, ip_addr='', netmask=None, node_port=51348,
        ...
        if scheduler_ip_addrs and scheduler_port:
            ...
            bc_sock.sendto(b'PING:' + serialize(relay_request), ('<broadcast>', node_port))
        bc_sock.close()
        ...
        while True:
            ...
            for sock in ready:
                if sock == node_sock:
                    ...
                    bc_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                    bc_sock.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)
                    bc_sock.sendto(b'PING:' + serialize(info), ('<broadcast>', node_port))
                    bc_sock.close()
                else:
                    assert sock == sched_sock
                    msg, addr = sched_sock.recvfrom(1024)
                    if msg.startswith(b'PING:') and scheduler_ip_addrs and scheduler_port:
                        try:
                            info = unserialize(msg[len(b'PING:'):])
