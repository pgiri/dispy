...
def _dispy_setup_process(compute, pipe, client_globals):
    ...
    if compute.code:
        try:
            if compute.code:
                exec(marshal.loads(compute.code)) in globals()
            if os.name != 'nt':
                compute.code = None
        except Exception:
            exc = sys.exc_info()
    ...
    if compute.setup:
        ...
        try:
            exec('_dispy_setup_status = %s(*_dispy_setup_args)' %
                 compute.setup) in globals(), localvars
    ...
    if isinstance(compute.cleanup, str):
        ...
        try:
            exec('%s(*_dispy_cleanup_args)' % compute.cleanup) in globals(), localvars
        except Exception:
...
class _DispyNode(object):
        ...
        def setup_computation(msg, task=None):
            ...
            if (not client.use_setup_proc):
                ...
                if compute.setup and isinstance(client.setup_args, tuple):
                    ...
                    sys.path.insert(0, compute.dest_path)
                    exec('_dispy_setup_status = %s(*_dispy_setup_args)' %
                         compute.setup) in globalvars, localvars
                    try:
    ...
    def timer_proc(self, task=None):
        ...
        while 1:
            ...
            if self.pulse_interval and (now - last_pulse_time) >= self.pulse_interval:
                ...
                resend = [client for client in self.clients.itervalues()
    ...
    def cleanup_computation(self, client, close=True, task=None):
        ...
        terminate_tasks = []
        for job_info in self.job_infos.itervalues():
        ...
        if isinstance(compute.cleanup, str) and isinstance(client.cleanup_args, tuple):
            ...
            try:
                ...
                if compute.code:
                    exec(marshal.loads(compute.code)) in globalvars
                exec('%s(*_dispy_cleanup_args)' % compute.cleanup) in globalvars, localvars
            except Exception:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--init', dest='node_init', type=str, default='',
                        help='path to file with "init" and/or "close" to initialize node')
